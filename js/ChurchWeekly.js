...
function navigateToDetail(id) {
  window.location.hash = `weekly-${id}`;
  showDetail(id);
}

// 이미지 경로 수정 함수는 weeklyData.json에서 이미 상대 경로를 사용하고 있으므로 제거

function showDetail(id) {
  const item = weeklyData.find((weekly) => weekly.id === id);
  if (!item) return;

  // 상세 페이지로 이동
  document.getElementById("weekly-list").style.display = "none";
...
function showList() {
  ...
  document.getElementById("weekly-list").style.display = "block";
  document.getElementById("weekly-detail").style.display = "none";
  renderTable(currentPage);
}

// loadweeklyData(); // <--- 이 부분이 제거됨
